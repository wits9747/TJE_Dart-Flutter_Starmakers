...
exports.deleteCompleteMessageData = functions.firestore
...
exports.deleteMessageChatDocs = functions.firestore
  .document("/messages/{messageId}")
  .onDelete(async (snap, context) => {
    const { messageId } = context.params;
    return db
      .collection(`messages/${messageId}/${messageId}`)
      .get()
      .then((res) => {
        res.forEach((element) => {
          element.ref.delete();
...
exports.deleteGroupChatDocs = functions.firestore
  .document("/groups/{groupId}")
  .onDelete(async (snap, context) => {
    const { groupId } = context.params;
    return db
      .collection(`groups/${groupId}/groupChats`)
      .get()
      .then((res) => {
        res.forEach((element) => {
          element.ref.delete();
...
exports.deleteBroadcast = functions.firestore
...
exports.deleteBroadcastChatDocs = functions.firestore
  .document("/broadcasts/{broadcastId}")
  .onDelete(async (snap, context) => {
    const { broadcastId } = context.params;
    return db
      .collection(`broadcasts/${broadcastId}/broadcastChats`)
      .get()
      .then((res) => {
        res.forEach((element) => {
          element.ref.delete();
...
exports.recieveGroupMsgNotification = functions.firestore
  .document("/groups/{groupId}/groupChats/{chatId}")
  .onCreate(async (snap, context) => {
    ...
    const payload = {
      notification: {
        ...
        body:
          ...
          ":  " +
          (message.type == 0
            ? multiLangNotifMap["ntm"]
            : message.type == 1
            ? multiLangNotifMap["nim"]
            : message.type == 2
            ? multiLangNotifMap["nvm"]
            : message.type == 3
            ? multiLangNotifMap["ndm"]
            : message.type == 4
            ? multiLangNotifMap["nlm"]
            : message.type == 5
            ? multiLangNotifMap["ncm"]
            : message.type == 6
            ? multiLangNotifMap["nam"]
            : multiLangNotifMap["ntm"]),
    ...
    await admin
      .messaging()
      .sendToTopic(`GROUP${message.iDfltrd}`, payload, options);
  });

exports.manageGroupTokens = functions.firestore
  .document("/tempUnsubscribeTokens/{id}")
  .onCreate(async (snap, context) => {
    const message = snap.data();
    ...
    if (message.type == "subscribe") {
      ...
      admin
        .messaging()
        .subscribeToTopic(registrationTokens, `GROUP${message.iDfltrd}`)
        .then((response) => {
      ...
      admin
        .messaging()
        .unsubscribeFromTopic(registrationTokens, `GROUP${message.iDfltrd}`)
        .then((response) => {
          // See the MessagingTopicManagementResponse reference documentation
          // for the contents of response.
...
exports.sendNewMessageNotification = functions.firestore
  .document("/messages/{chatId}/{chat_id}/{timestamp}")
  .onCreate(async (snap, context) => {
    const message = snap.data();
    if (message.isMuted == true) {
    } else {
      // Get the list of device notification tokens.
      const getRecipientPromise = admin
        .firestore()
        .collection("users")
        .doc(message.to)
        .get();
      ...
      isMultilangNotificationEnabled =
        recipient.data().isMultiLangNotifEnabled || false;
...
exports.newIncomingCall = functions.firestore
          ...
          if (recipient.data().deviceDetails["OS type"] == "android") {
            payload = {
              ...
              data: {
                ...
                bodyMultilang:
                  message["CALLERNAME"] || recipient.data().nickname,
...
exports.callRejectedFirstTime = functions.firestore
  .document("/users/{userId}/recent/callended")
  .onCreate(async (snap, context) => {
    const message = snap.data();
...
exports.callRejectedNotFirstTime = functions.firestore
  .document("/users/{userId}/recent/callended")
...
exports.createCallsWithTokens = functions.https.onCall(
  async (data, context) => {
    try {
